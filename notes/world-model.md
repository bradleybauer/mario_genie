...
In this repo, the LTX side is real code today: the implemented pieces are the video compressor in ltx_video_vae.py, the audio compressor in ltx_audio_vae.py, and the mel-to-waveform decoder in ltx_audio_vocoder.py. The transformer world-model part is still a design in ai-notes.md, not an implemented module under src yet. The tests in test_vaes.py confirm the current latent contracts.
