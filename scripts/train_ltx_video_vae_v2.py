#!/usr/bin/env python3
"""Train the LTX Video VAE v2 (learned-upsample decoder)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the LTX Video VAE v2 (learned-upsample decoder).")
...
def make_output_dir(args: argparse.Namespace) -> Path:
    ...
    run_name = args.run_name or datetime.now().strftime("ltx_video_vae_v2_%Y%m%d_%H%M%S")
...
def main() -> None:
    ...
    console.print(f"Training LTX Video VAE v2 on {len(train_dataset)} samples")
        ...
        for step in range(start_step, args.max_steps):
            ...
            if should_eval:
                ...
                if eval_metrics["loss"] < best_eval:
                    ...
                    save_training_state(
                        output_dir / "video_vae_v2_best.pt",
                        model=model,
                        optimizer=optimizer,
                        scheduler=scheduler,
                        step=step,
            ...
            if should_checkpoint:
                ...
                save_training_state(
                    output_dir / "video_vae_v2_latest.pt",
                    model=model,
                    optimizer=optimizer,
                    scheduler=scheduler,
                    step=step,
