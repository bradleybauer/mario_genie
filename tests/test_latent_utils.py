...
class DummyCFGModel(torch.nn.Module):
    def encode_history(
        ...
        action_term = history_actions.float().sum(dim=1).to(dtype=history_latents.dtype)
    ...
    def decode_future(
        ...
        action_term = actions.float().sum(dim=1).to(dtype=noisy_future.dtype).view(batch, 1, 1, 1, 1)
...
def _run_segment(scale: float) -> torch.Tensor:
    ...
    actions = torch.tensor([[1, 2, 3], [4, 5, 6]], dtype=torch.long)
